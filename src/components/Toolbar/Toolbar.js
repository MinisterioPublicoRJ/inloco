...
const Toolbar = ({showSidebarRight, onToolbarItemClick, toolbarActive, ownProps, layers, places}) => {
    let className
    let active
                ...
                items.map( (item, index) => {
                    ...
                    return (
                        <div data-id={item.name} key={index} className={itemClassName} onClick={(e) => handleClick(e)}>
                            <ToolbarMenu item={item} active={active} type={type} layers={layers} places={places}> </ToolbarMenu>
                        </div>)
                })
            }
        </div>
